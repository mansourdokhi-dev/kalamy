...
5. Write an `AuditLog` entry matching the shape already used elsewhere in this module (`specialist-review.service.ts:210,320`): `userId: actor.id`, `action: 'RESTART_CYCLE_AFTER_INACTIVITY'`, `entity: 'TrainingCycle72h'`, `entityId: <old cycle id>`, `before: { status: 'CLOSED_DUE_TO_INACTIVITY' }`, `after: { newCycleId: <new cycle id>, levelId }` — this is a sensitive clinical action and CLAUDE.md's governing rule requires sensitive operations to be audit-logged.
