...
2. "X ساعة متبقية كحد أدنى قبل مرحلة العينة" (X hours remaining as a minimum before the sample stage) — computed from `cycle.firstTrainingEventAt`; if null, shows the full 72h; if the 72h has already elapsed, this line is omitted. **Correction from an earlier draft of this design**: the actual backend eligibility check (`cycle-eligibility.util.ts`) requires both the 72h elapsed *and* at least one training event in each of the three 24h periods (or a make-up event after 72h if a period was missed) — 72h elapsed alone is necessary but not sufficient, so a patient can genuinely still be `ACTIVE_LEVEL_TRAINING` past the 72h mark. The copy is worded as a minimum-time indicator, not a promise that the stage opens the instant the counter reaches zero, to avoid the exact "training count/time alone implies success" framing §62 explicitly warns against.
